...
// Patches a bug in goravel/s3 v1.18.0 where "use_path_style": true in the
// disk config was silently ignored (requests always went out
// virtual-hosted-style) - see third_party/goravel-s3/s3.go for the fix.
// Path-style is required for our custom S3-compatible endpoint (MinIO /
// the production CDN), which doesn't support bucket-as-subdomain routing.
replace github.com/goravel/s3 => ./third_party/goravel-s3
